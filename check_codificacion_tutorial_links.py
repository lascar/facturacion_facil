#!/usr/bin/env python3
"""
Script pour vérifier tous les liens dans le tutorial de codificación avancée
"""
...
def check_codificacion_tutorial_links():
    """Vérifie tous les liens dans le dossier tutorial_codificacion_html"""
    tutorial_dir = Path("doc/tutorial_codificacion_html")
    ...
    print("📁 Fichiers HTML existants dans tutorial_codificacion_html:")
    ...
    print("\n" + "="*60)
    print("📈 PLAN DE CRÉATION DES FICHIERS MANQUANTS:")
    
    if missing_files:
        print("\nFichiers à créer pour compléter le tutorial:")
        for file in sorted(missing_files):
            title = file.replace('.html', '').replace('-', ' ').title()
            print(f"  📄 {file} - {title}")
    else:
        print("✅ Tutorial complet - tous les fichiers existent!")
    
    return broken_links, missing_files

if __name__ == "__main__":
    print("🔍 Vérification des liens du tutorial de codificación avancée...")
    print("="*60)
    
    broken_links, missing_files = check_codificacion_tutorial_links()
    ...
    if missing_files:
        print(f"\n🎯 PRÓXIMOS PASOS:")
        print(f"  1. Crear los {len(missing_files)} archivos faltantes")
        print(f"  2. Implementar el contenido técnico avanzado")
        print(f"  3. Verificar que todos los enlaces funcionen")
        print(f"  4. Revisar la coherencia del tutorial completo")
    else:
        print(f"\n🎉 ¡TUTORIAL COMPLETO!")
        print(f"  ✅ Todos los enlaces funcionan correctamente")
        print(f"  ✅ Navegación completa implementada")
        print(f"  ✅ Listo para uso por desarrolladores avanzados")
