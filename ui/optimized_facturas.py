...
"""
Interface de facturas optimisee pour de meilleures performances
"""
...
class OptimizedFacturasWindow(BaseWindow):
    """Fentre de facturas optimisee avec chargement paresseux et cache"""
    
    def __init__(self, parent, nueva_factura=False):
        super().__init__(parent, get_text("facturas"), "1400x900")
        
        # Variables de performance
        self.facturas_summary = []  # Resume pour l'affichage rapide
        self.facturas_full = {}     # Cache des facturas compltes
        self.selected_factura_id = None
        self.current_factura = None
        self.productos_cache = {}   # Cache des productos
        
        # Variables de recherche optimisee
    ...
    def create_facturas_list_optimized(self, parent):
        """Creer la liste de facturas optimisee"""
        # Titulo y controles
        ...
        self.search_entry = ctk.CTkEntry(
            ...
            placeholder_text="Buscar por numero, cliente...",
            width=250
        )
        self.search_entry.pack(side="left", padx=5)
        
        # Busqueda con debouncing
        ...
        # Configurar columnas
        self.facturas_tree.heading("numero", text="Numero", command=lambda: self.sort_facturas("numero"))
        ...
        # Bind para seleccion optimizada
    ...
    def create_factura_details_optimized(self, parent):
        """Creer le panel de details de factura optimise"""
        # Titulo
        header_frame = ctk.CTkFrame(parent)
        header_frame.pack(fill="x", padx=10, pady=(10, 5))
    ...
    def load_facturas_optimized(self):
        """Charger les facturas de manire optimisee (resume seulement)"""
        try:
            self.loading_label.configure(text=" Cargando...")
            self.window.update()
            
            # Charger seulement le resume pour l'affichage rapide
    ...
    def on_factura_select_optimized(self, event):
        """Gerer la selection de factura de manire optimisee (chargement paresseux)"""
        try:
            selection = self.facturas_tree.selection()
            if not selection:
                return
            
            # Obtenir l'ID de la factura selectionnee
            item = self.facturas_tree.item(selection[0])
            factura_id = int(item['tags'][0])
            
            if factura_id == self.selected_factura_id:
                return  # Dej selectionnee
            
            self.selected_factura_id = factura_id
            
            # Charger les details de manire asynchrone
            self.load_factura_details_async(factura_id)
            
        except Exception as e:
            self.logger.error(f"Error en seleccion de factura: {e}")
    
    def load_factura_details_async(self, factura_id):
        """Charger les details de factura de manire asynchrone"""
        try:
    ...
    def display_factura_details(self, factura):
        """Afficher les details de la factura"""
        try:
            # Nettoyer le frame de details
            ...
            info_data = [
                ...
                ("Telefono:", factura.telefono_cliente or "N/A"),
    ...
    def load_productos_cache(self):
        """Charger le cache des productos pour eviter les requtes repetees"""
        try:
    ...
    def delayed_search(self, search_time):
        """Executer la recherche avec delai"""
    ...
    def show_no_selection_message(self):
        """Afficher un message quand aucune factura n'est selectionnee"""
        for widget in self.details_frame.winfo_children():
            widget.destroy()
    ...
    def nueva_factura(self):
        """Creer une nouvelle factura"""
        # Implementer la creation de nouvelle factura
        self.show_info_message("Nueva Factura", "Funcionalidad en desarrollo")
    
    def eliminar_factura(self):
        """Supprimer la factura selectionnee"""
